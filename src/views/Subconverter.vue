              ...
              <div v-if="advanced === '2'">
                ...
                <el-form-item label="远程配置:">
                  <el-select v-model="form.remoteConfig" allow-create filterable placeholder="请选择" style="width: 100%">
                    <el-option-group v-for="group in remoteConfig" :key="group.label" :label="group.label">
                      <el-option v-for="item in group.options" :key="item.value" :label="item.label"
                        :value="item.value"></el-option>
                    </el-option-group>
                    <el-button slot="append" @click="gotoRemoteConfig" icon="el-icon-link">配置示例</el-button>
                    <el-button slot="append" @click="$router.push('/setting')" icon="el-icon-setting">配置管理</el-button>
                  </el-select>
                </el-form-item>
...
export default {
  ...
  mounted() {
    ...
    // 在 mounted 时初始化远程配置
    this.$store.commit('INIT_REMOTE_CONFIG')
  },
  methods: {
    onCopy() {
 ...
 */
    ...
    backendSearch(queryString, cb) {
      const backends = this.backendOptions || [];
      const results = queryString
        ? backends.filter(backend => 
            backend.value.toLowerCase().includes(queryString.toLowerCase())
          )
        : backends;
      // 确保返回数组
